...
class AlipayNotify
{
    ...
    public function __construct()
    {
        $this->alipay_config = array(
            ...
            "private_key_path" => Yii::getAlias("@third_party/Alipay/key/rsa_private_key.pem"),
            "ali_public_key_path" => Yii::getAlias("@third_party/Alipay/key/alipay_public_key.pem"),
            "sign_type" => strtoupper('RSA'),
            "input_charset" => strtolower('utf-8'),
            "cacert" => Yii::getAlias("@third_party/Alipay/key/cacert.pem"),
            "transport" => 'http',
        );
    }
    ...
    public function verifyReturn()
    {
        if (empty($_GET)) {
            ...
            $this->logResult("isSign:" . $isSign);
            $responseTxt = 'true';
            if (!empty($_GET["notify_id"])) {
                $responseTxt = $this->getResponse($_GET["notify_id"]);
            }
            $this->logResult("responseTxt:" . $responseTxt);
            if (preg_match("/true$/i", $responseTxt) && $isSign) {
                return true;
            } else {
                return false;
    ...
    public function logResult($word = '')
    {
        ...
        $fp = fopen($path . '/alipay_notify_log.txt', "a");
        flock($fp, LOCK_EX);
